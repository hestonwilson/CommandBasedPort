...
SwerveModule::SwerveModule(frc::Translation2d pos, int analogEncoderPort, units::radian_t analogEncoderOffset, int driveMotorCANID, int turnMotorCANID, SwerveModuleName moduleName)
  ...
  m_driveEncoder.SetPositionConversionFactor(SDS_WHEEL_DIAMETER.to<double>() * PenguinConstants::MathConstants::PI / SDS_DRIVE_REDUCTION); // so this is meters, right? // or, I guess inches? (wheel diameter is inches, conversion multiplier is circumference/reduction) huh. // TODO: make sure this is returning meters
  m_driveEncoder.SetVelocityConversionFactor(SDS_WHEEL_DIAMETER.to<double>() * PenguinConstants::MathConstants::PI / SDS_DRIVE_REDUCTION * (1.0 / 60.0)); // SDS: "RPM to units per sec" // TODO: make sure this is returning m/s
