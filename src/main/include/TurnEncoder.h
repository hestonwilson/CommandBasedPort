 ...
 public:
  TurnEncoder(int port, units::radian_t offset, rev::CANEncoder sparkMaxEncoder) : 
    ...
    offsetRot{offset} {
      ...
      builtInMotorEncoder.SetPositionConversionFactor(INVERT_BUILTIN_ENCODER_I_THINK * PenguinConstants::MathConstants::TWO_PI / (18. / 1.)); // 18:1 is the default angle reduction
  ...
  units::radian_t GetAngle(bool continuous = false) const {
    units::radian_t angle = (1.0 - encoderAsAnalogInput.GetVoltage() / frc::RobotController::GetVoltage5V()) * PenguinConstants::MathConstants::TWO_PI_RAD;
    angle += offset;
    // angle *= -1;

    if (!continuous) {
      angle = units::math::fmod(angle, PenguinConstants::MathConstants::TWO_PI_RAD);


      while (angle > PenguinConstants::MathConstants::PI_RAD) {
        angle -= PenguinConstants::MathConstants::TWO_PI_RAD;
      }
      while (angle < -PenguinConstants::MathConstants::PI_RAD) {
        angle += PenguinConstants::MathConstants::TWO_PI_RAD;
      }
